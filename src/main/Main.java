...
import main.Main.Modifiers.Mirroring;
import main.Main.Modifiers.Polarity;
	...
	private void processCommand(Command command)
	{
			...
			if (inRegion)
			{
				switch (apertureNumber)
				{
					...
					case 2:
						...
						renderer.objects.add(new Region(p, globalModifiers.clone()));
		...
		else if (commandWord.startsWith("G37"))
		{
			...
			if (currentRegion != null)
			{
				...
				// Check if closed
				if (currentRegion.get(0) != currentRegion.get(currentRegion.size() - 1))
				{
					throw new RuntimeException("Attempt to close an open region.");
				}
